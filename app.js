...
document.getElementById('tabContainer').addEventListener('click', e => {
    ...
    if (!btn) return;
    document.querySelectorAll('.type-tile').forEach(b => b.classList.remove('active'));
...
document.getElementById('themePresets').addEventListener('click', e => {
    ...
    if (!p) return;
    document.querySelectorAll('.preset-swatch').forEach(b => b.classList.remove('active-preset'));
...
function renderHistory() {
    ...
    list.querySelectorAll('.history-item').forEach(el => {
        el.addEventListener('click', () => {
            ...
            const btn = document.querySelector(`.type-tile[data-type="${item.type}"]`);
            if (btn) btn.click();
